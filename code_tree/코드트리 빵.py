...
def find(si, sj, dests):    # 시작좌표에서 목적지좌표들(set) 중 최단거리 동일반경 리스트를 모두 찾기
    ...
    while q:
        ...
        nq = deque()
        for ci, cj in q:
            if (ci, cj) in dests:    # 목적지 찾음 => 더 뻗어나갈 필요 없음
                tlst.append((ci, cj))
            else:
                # 네방향, 미방문, 조건: 벽이아니면 arr[][]==0
...
def solve():
    ...
    while q or time==1:    # 처음 또는 q에 데이터가 있는동안(이동할 사람이 있는동안) 실행
        nq = deque()
        alst = []

        # [1] 모두 편의점방향으로 최단거리 이동 (이번 time만, 같은반경)
        for ci, cj, m in q:
            if arrived[m] == 0: # 도착하지 않은사람만 처리
                # 편의점방향 최단거리(우선순위) 한 칸 이동
                # 편의점에서 시작, 현재위치의 상하좌우 => dests (set)
                ni, nj = find(store[m][0], store[m][1], set([(ci-1, cj), (ci+1, cj), (ci, cj-1), (ci, cj+1)]))
                if (ni, nj) == store[m]:    # 최종 편의점에 도착
                    arrived[m] = time
                    alst.append((ni, nj))   # 통행금지는 모두 이동후 처리해야함
                else:
                    nq.append((ni, nj, m))  # 계속이동
